function [Reward_summary,Vi_hat,PSi_p_hat,Value_learned_Node_2] = TSOR_DE(T,R,N,PSi,Obs,sigma_t,n_max,A,A_p,c_i,r_i)
...
for j = 1:N
    br = betarnd(cell2mat(M_alpha(j)),cell2mat(M_beta(j)));
    br = br / sum(br);
    PSi_p_hat(j) = {br};
end
% Initial_p_hat = PSi_p_hat;
%% At each time step
for i = 1:n_max
...
%         end
    ...
    for j = 1:numel(current_packets)
        ...
        if Cost_summary(packet_index) > R
            ...
            up_num = up_num + 1;
            UP_index(up_num) = 1;
            UP_Loc(up_num, current_location) = 1;
            continue;
        end
        ...
        if psi_update_flag(current_location) == 0
            ...
            br = betarnd(cell2mat(M_alpha(current_location)),cell2mat(M_beta(current_location)));
            br = br / sum(br);
            PSi_p_hat(current_location) = {br};
...
%                             diff = abs(Vi_hat(neighbor_index, current_location) - Vi_hat(current_location, current_location));
                            locs = UP_Loc(up_index,:);
%                             if Vi_hat(neighbor_index, current_location) ~= Vi_hat(current_location, current_location)
